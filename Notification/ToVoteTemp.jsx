	...
	componentDidMount(){
		...
		$.ajax({
			...
			success: function(result){
                ...
                var controlArray = [];
                for(var i = 0; i < result.controlTokenQuantity.length; i++)
                {
                    var temp = [];
                    temp[0] = result.controlIdList[i];
                    temp[1] = result.controlTokenQuantity[i];    
                    
                    controlArray.push(temp)
                    
                }
                
                //now add controlArray to the JSON object.
                result.controlArray = controlArray
                console.log(result.controlArray)
						...
						<tbody>
							...
							<tr>
								...
								if(!$.isEmptyObject(this.state.proposal_data)){
									...
									return( <tr key={i}> 
                                                         <td><p><b>{ids[0]}</b></p><p>The owner above has the ownership stake: {ids[1]} </p></td>
                                            </tr>)
									})
								}
								})(this)}
							...
							<tr>
								...
								<td>{(() => {
								if(!$.isEmptyObject(this.state.proposal_data)){
									return this.state.proposal_data.controlArray.map((ids,i) => {
									return( <tr key={i}> 
                                                    <td><p><b>{ids[0]}</b></p><p>The controller above has the control stake: {ids[1]} </p></td>
                                            </tr>)
									})
								}
								})(this)}
