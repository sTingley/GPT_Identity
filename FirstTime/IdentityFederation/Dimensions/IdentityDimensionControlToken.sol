...
contract IdentityDimensionControlToken {
    ...
    //eventually we will need to account for several blockchain accounts
    bytes32 chairperson;
    ...
    bytes32[10] owners;
    bytes32[10] controllers;

    //There are only ten owners, because a core identity has maximum ten owners.
    function IdentityDimensionControlToken(bytes32[10] owners_initial, bytes32[10] controllers_initial) {
    ...
    function resetControllers(bytes32[10] newControllers) access(msg.sender) {
        controllers = newControllers;   
    }

    function addController(bytes32 owner, bytes32 newControllers) access(msg.sender) returns (bool success) {
        ...
        if (success) {
            for (uint i=0;i < controllers.length;i++) {
                if (!added && controllers[i] == 0x0) {
                    controllers[i] = newControllers;
                    added = true;
                }           
            }
        }
    ...
    function removeController(bytes32 owner, bytes32 newControllers) access(msg.sender) returns (bool removed) {
        ...
        if (success) {
            for (uint i=0;i < controllers.length;i++) {
                if (!removed && controllers[i] == newControllers) {
                    controllers[i] = 0x0;
                    removed = true;
                }           
            }
    ...
    function controllerExists(bytes32 controllerHash) access(msg.sender) returns (bool exists, uint index) {
        exists = false;
        index = 0;
    ...
    function isController(bytes32 controllerHash) access(msg.sender) returns (bool exists) {
        exists = false;
        uint index = 0;
    ...
    function addDelegation(bytes32 owner, bytes32 delegatee, uint amount, string dimension, uint timeFrame, string accessCategories) access(msg.sender) returns (bool success) {
        ...
        if (!success) {
            (success,index) = this.controllerExists(owner);
        }
    ...
    function replaceOwnerController(bytes32 oldOwner, bytes32 newOwner){
        if (delegations.length > 0) {
            for (uint i = 0; i < delegations.length; i++) {
                if (delegations[i].owner == oldOwner) {
                    delegations[i].owner = newOwner;
                }
            }
        }
    }

    function replaceDelegatee(bytes32 oldDelegatee, bytes32 newDelegatee){
        if (delegations.length > 0) {
            for (uint i = 0; i < delegations.length; i++) {
                if (delegations[i].delegatee == oldDelegatee) {
                    delegations[i].delegatee = newDelegatee;
                }
            }
        }
    }
    ...
    //tells you how many tokens a delegatee has for a given identityDimension in order to access a specific descriptor
